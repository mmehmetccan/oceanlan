...
const FeedPage = () => {
  ...
  const handleRemoveFriend = async (targetUserId) => {
      try { await axiosInstance.post('/friends/remove', { targetUserId }); setFriends(prev => prev.filter(f => f._id !== targetUserId)); setPendingRequests(prev => prev.filter(req => req.recipient?._id !== targetUserId)); addToast('Kişi silindi', 'success'); } catch (err) { addToast('Hata oluştu', 'error'); }
  };
  ...
  const handleMenuAction = (action, targetUser) => {
      ...
      if (action === 'remove' || action === 'cancel') { setConfirmModal({ isOpen: true, title: action === 'remove' ? 'Arkadaşı Çıkar' : 'İsteği İptal Et', message: 'Emin misin?', isDanger: true, confirmText: 'Evet', onConfirm: () => handleRemoveFriend(targetUser._id) }); }
      if (action === 'accept' || action === 'reject') { const req = pendingRequests.find(r => r.requester?._id === targetUser._id); if(req) handleFriendResponse(req._id, action === 'accept' ? 'accepted' : 'rejected'); }
  };
  ...
  const getStatusText = (friend) => {
    ...
    if (friend.lastSeenAt) { const dt = new Date(friend.lastSeenAt); return `Son: ${dt.toLocaleDateString('tr-TR', { day: 'numeric', month: 'short' })} ${dt.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}`; }
    return 'Çevrimdışı';
  };
      ...
      <aside className="feed-rail">
        ...
        <div className="rail-card">
          ...
          <form className="rail-form" onSubmit={handleSendFriendRequest}>
            <input type="text" className="rail-input" placeholder="Kullanıcı adı..." value={recipientUsername} onChange={e => setRecipientUsername(e.target.value)} />
