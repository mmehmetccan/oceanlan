...
// Google'da sorun olursa Cloudflare devreye girer.
const rtcConfig = {
  iceServers: [
    // Google (Standart)
...
export const useVoiceChannel = () => {
  ...
  useEffect(() => {
    ...
    const startVoiceConnection = async () => {
      ...
      // Zaman aşımı kontrolü (15 saniye - Artırıldı)
      connectionTimeoutRef.current = setTimeout(() => {
        if (isMounted && !localStreamRef.current) {
          addToast('Ses kanalına bağlanmak uzun sürüyor, lütfen bekleyin veya sayfayı yenileyin...', 'warning');
        }
      }, 15000);
