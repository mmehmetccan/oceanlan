...
export const VoiceProvider = ({ children }) => {
  ...
  const startScreenShare = async (electronSourceId = null) => {
    try {
        ...
        // 🟢 2. KRİTİK ADIM: Mevcut Peer Bağlantılarına Yayını Ekle
        // Sadece kendi ekranımda görmek yetmez, diğerlerine de göndermeliyim.
        Object.values(peersRef.current).forEach(peer => {
            try {
                // Eğer peer bağlantısı varsa yayını ekle
                if (peer && !peer.destroyed) {
                    peer.addStream(stream);
                }
            } catch (err) {
                console.error("Yayın gönderme hatası:", err);
            }
        });

        // Yayın bittiğinde temizle
  ...
  const stopScreenShare = () => {
      if (myScreenStream) {
          ...
          // Mevcut peerlardan yayını kaldır
          Object.values(peersRef.current).forEach(peer => {
              try {
